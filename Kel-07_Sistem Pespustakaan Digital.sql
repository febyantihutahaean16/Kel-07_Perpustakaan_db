...
-- 2. MEMBUAT TRIGGER INSERT DATA
	...
	FOR EACH ROW EXECUTE FUNCTION cek_anggota();
		...
		INSERT INTO anggota (nama, alamat, telepon) VALUES ('Alice Johnson', 'Jl. Merdeka', '08765432101');
		-- Expected output: NOTICE: Anggota dengan nama Alice Johnson sudah ada

			-- Verifikasi tabel anggota
			SELECT * FROM anggota;

		
		INSERT INTO anggota (nama, alamat, telepon) VALUES ('Michael Jordan', 'Jl. Basket 23', '08987654321');
		-- Expected output: Data berhasil disisipkan tanpa peringatan
	...
	FOR EACH ROW EXECUTE FUNCTION cek_buku();
		...
		-- 3. Menyisipkan data dengan stok negatif
		INSERT INTO buku (judul, penulis, penerbit, tahunTerbit, stok, kategori_id)
		...
		-- 4. Menyisipkan buku dengan judul dan penulis yang sama
		INSERT INTO buku (judul, penulis, penerbit, tahunTerbit, stok, kategori_id)
...
-- 3. Membuat Transaction dan Trigger untuk  Proses Peminjaman Buku, Proses Pengembalian Buku, dan Proses Pembayaran Denda
	...
	select proses_pengembalian_transaksi(6, '2024-12-5');

		-- Memeriksa isi data dalam tabel
		SELECT * FROM log_peminjaman;
		
	-- Tepat waktu
	select proses_pengembalian_transaksi(6, '2024-12-7');

		-- Memeriksa isi data dalam tabel
		SELECT * FROM log_peminjaman;
	-- Terlambat
	select proses_pengembalian_transaksi(6, '2024-12-12');
